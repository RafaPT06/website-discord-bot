...
function mobileBreadcrumb(server, active = 'Dashboard') {
  if (!server) return '';
  return `<div class="dash-mobile-breadcrumb"><a href="/dashboard">Dashboard</a><span>›</span><a href="${escapeHtml(sectionPath(server))}">${escapeHtml(server.name)}</a><span>›</span><strong>${escapeHtml(active)}</strong></div>`;
}
...
function channelField(name, label, value, fallback = '# general') {
  const channels = ['# welcome', '# level-up', '# logs', '# mod-logs', '# general'];
  const current = String(value || fallback);
  if (!channels.includes(current)) channels.unshift(current);
  return `<label class="dash-field"><span>${escapeHtml(label)}</span><select name="${escapeHtml(name)}">${channels.map((channel) => `<option value="${escapeHtml(channel)}" ${channel === current ? 'selected' : ''}>${escapeHtml(channel)}</option>`).join('')}</select><small>Local preview selector until live Discord channel sync is connected.</small></label>`;
}
